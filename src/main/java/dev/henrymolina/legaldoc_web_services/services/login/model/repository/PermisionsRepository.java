...
public interface PermisionsRepository extends JpaRepository<Permisions, Long> {
    @Query(value = "select p from User u join UserTypePermission utp on utp.idTipoUsuario = u.userType.id join UserType ut on ut.id = utp.idTipoUsuario join Permisions p on p.id = utp.idPermiso WHERE utp.habilitado is true and p.habilitado is true and u.correo = ?1")
